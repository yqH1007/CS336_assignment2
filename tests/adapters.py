...
from cs336_systems.ddp import DDP,DDPOverlap
...
def get_ddp(module: torch.nn.Module) -> torch.nn.Module:
    ...
    return DDPOverlap(module=module)
...
def get_fsdp(module: torch.nn.Module, compute_dtype: torch.dtype | None = None) -> torch.nn.Module:
    ...
    return FSDP(module=module,compute_dtype=compute_dtype)
...
def fsdp_gather_full_params(fsdp_model: torch.nn.Module) -> dict[str, torch.Tensor]:
    ...
    for name,param in fsdp_model.module.named_parameters():
        if param in fsdp_model.param_to_module:
            m = fsdp_model.param_to_module[param]
            full = torch.empty(m._fsdp_full_shape,device=param.device,dtype=param.dtype)
